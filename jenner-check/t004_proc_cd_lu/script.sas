/*--------------------------------------------------------------------*\
| Bundle: exercise the procedure-code grouper builder from              |
| amdg/00_common/00_proc_cd_lu.sas. The DATA step below is copied from   |
| the repo's 00_proc_cd_lu.sas with only the target library rewritten    |
| (inp.r&vz._proc2cat_lu -> work.proc2cat_lu) so it runs standalone;     |
| the CARDS payload (the clinical proc-code -> category mapping) and the  |
| SCAN/TRANWRD parsing are unchanged. A PROC FREQ summarises the         |
| resulting category counts.                                            |
\*--------------------------------------------------------------------*/

data proc2cat_lu(drop=line); 
input line $char60.;
...
proc freq data=proc2cat_lu order=freq;
   tables proc_cd_cat / nocum;
   title "00_proc_cd_lu: procedure-code category groupers";
run;

proc print data=proc2cat_lu(obs=8) noobs;
   var proc_cd proc_cd_cat;
   title "00_proc_cd_lu: first rows of the lookup";
run;
